    ...
    <tab-pane label="山体阴影">
      <div style="margin-bottom: 10px">
        <div style="text-align: center;padding-top: 10px">
          ...
          <button class="btn btn-info btn-sm" @click="addResult(0)">山体阴影</button>
...
export default {
  ...
  methods:{
    ...
    coordinatePost() {
      let that = this
      axios.post(`http://127.0.0.1:5000/hillShade`, this.postData).then(res => {
    ...
    downTif(num) {
      window.open(`http://127.0.0.1:5000/static/tempoutput/${this.downLoad[num]}`);
    },
    //查询tif像素值
    readingPixel() {
      if (imageStores.imageStore.length == 0) {
        ...
        drawShape.drawPoint({
          ...
          endCallback: function (ps) {
            ...
            //http://122.227.134.126:98/QGIS_readingPixel/readingPixel
            //http://127.0.0.1:4079/readingPixel
            axios.post(`http://122.227.134.126:98/QGIS_readingPixel/readingPixel`, store).then(res => {
              console.log("添加数据返回结果为：", res)
    ...
    async addResult(num) {
      //获取tif
      const response = await fetch(`http://127.0.0.1:5000/static/tempoutput/${this.downLoad[num]}`);
