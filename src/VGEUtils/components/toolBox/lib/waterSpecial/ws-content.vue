  ...
  <win-tabs style="width: 350px;left:auto;right: 200px" @close="close">
    <tab-pane label="水面波纹配置">
      <table>
        <tr>
          ...
          <td style="text-align: left">
            <el-color-picker size="small" v-model="color" show-alpha @change="save"></el-color-picker>
          </td>
        </tr>
        <tr>
          <td>波纹频率：</td>
          <td><input type="range" v-model="water_freq" min="0" max="200" step="1" @change="save"></td>
        </tr>
        <tr>
          <td>波动速度：</td>
          <td><input type="range" v-model="water_animationspeed" min="0.001" max="0.01" step="0.001" @change="save"></td>
        </tr>
        <tr>
          <td>波动高度：</td>
          <td><input type="range" v-model="water_amplitude" min="1" max="5" step="0.1" @change="save"></td>
        </tr>
        <tr>
          <td>水体高度：</td>
          <td>
      ...
      <div style="text-align: center">
        <el-button type="success" size="small" style="margin-left: 10px" @click="save">保存</el-button>
        <el-button type="danger" size="small" @click="close">退出</el-button>
      </div>
    </tab-pane>
  </win-tabs>
</template>
