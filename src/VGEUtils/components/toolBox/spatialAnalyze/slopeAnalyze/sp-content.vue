<template>
    <win-tabs :initCSS="{width: 360,height: 210,left:500,top:330}" @close="close" v-loading="loading"
        ...
        <tab-pane label="坡度分析">
            <div @mousedown.stop>
                <div class="sp-content">
                    ...
                    <button class="btn btn-warning btn-sm" :disabled="!coordinates.length" @click="downTif">导出
                    </button>
...
export default {
    ...
    methods: {
        close() {
            this.$store.commit('setVGEEarthComAction', {name: 'slopeAnalyze', on_off: 2});
            this.removeImage();
        },
        ...
        excavate() {
            ...
            //绘制矩形
            // drawer = new VGEEarth.DrawShape(VGEEarth.getMainViewer());
        ...
        coordinatePost() {
            this.loading = true;
            axios.post(window.GISResourcesUrl + `/slopeAnalyze`, this.coordinates).then(res => {
                console.log('添加数据返回结果为：', res);
                this.tifId = res.data;
                let geoTiff = res.data;//返回的的结果ID
                this.addResult(geoTiff);//添加计算结果
            });
        },
        //添加到球上
        async addResult(geoTiff) {
            ...
            this.$store.commit('setLegendCurrent', {
                title: '坡度分析', list: [
                    {text: '0°-30°', color: 'rgb(255, 191, 191, 255)'},
                    {text: '30°-60', color: 'rgb(255, 128, 128, 255)'},
                    {text: '60°-90', color: 'rgb(255, 77, 77, 255)'},
                ], img: null
            });
        },
        //查询tif像素值
                ...
                drawShape.drawPoint({
                    ...
                    endCallback: (ps) => {
                        ...
                        axios.post(window.GISResourcesUrl + `/readingPixel`, store).then(res => {
                            console.log('添加数据返回结果为：', res);
                            this.pixelValue = res.data;//返回的的结果ID
                            this.simpleLabel(lonlat, this.pixelValue, height);
                        });
                    }
                });
            }
        ...
        //导出tif
        downTif() {
            window.open(window.GISResourcesUrl + `/static/tempoutput/aspect/${this.tifId}`);
        },
        //清空
        removeImage() {
            this.area = 0;
