      ...
      <div>
        ...
        <div class="vi-ctr">
          ...
          <div>
            ...
            <button class="btn btn-warning btn-sm" :disabled="imageLength === false" @click="downTif">导出</button>
            <button class="btn btn-warning btn-sm" :disabled="imageLength === false" @click="readingPixel()">取值</button>
...
export default {
  ...
  methods: {
    ...
    coordinatePost() {
      ...
      //服务器地址
      axios.post(`http://8.146.208.114:9001/viewAnalyze`, [this.coordinates, this.point[0][0], this.point[0][1],
        this.WatcherHigh, this.WatchHigh, this.VisualRange]).then(res => {
        this.tifId = res.data;
        let geoTiff = res.data;
        this.addResult(geoTiff);
      });
